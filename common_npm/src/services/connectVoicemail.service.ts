...
export class ConnectVoiceMail {
  ...
  async _getMedia(region: string, streamName: string, fragmentNumber: string) {
    // get data endpoint
    const kinesisvideo = new AWS.KinesisVideo({ region });
    ...
    // Reading raw content
    const kinesisvideomedia = new AWS.KinesisVideoMedia({ endpoint: end.DataEndpoint, region });
    var params = {
      StartSelector: {
